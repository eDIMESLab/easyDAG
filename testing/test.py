...
from easyDAG import _NO_PREVIOUS_RESULT
...
# %%
class TestClass:
    def test_eval_curry(self):
        a = InputVariable('a')
        b = InputVariable('b')
        e1 = do_eval(a+b, a=1)
        assert do_eval(e1, b=4) == 5
    
    def test_curry_different_expressions(self):
        a = InputVariable('a')
        b = InputVariable('b')
        c = InputVariable('c')
        d = InputVariable('d')
    ...
    def test_subclass_deferred_execution(self):
        class TestStep(Step):
    ...
    def test_equality_for_singletons(self):
        a = InputVariable('a')
        b = InputVariable('b')
        c = Singleton('a')    
        assert not are_equal(a, b)
    ...
    def test_variable_identity_independence(self):
        a = InputVariable('a')
        b = InputVariable('a')
        assert do_eval(a*b, a=5, b=3) == 25
    
    def test_programmatically_create_model(self):
    ...
    def test_data_structures_list(self):
        a = InputVariable('a')
        b = InputVariable('b')
        c = InputVariable('c')
        L = List(a, b, c)
    ...
    def test_data_structures_dict(self):
        a = InputVariable('a')
        b = InputVariable('b')
        D = DictKV(a, b)
        assert do_eval(D, a=['a', 'b'], b=[1, 2]) == {'a':1, 'b':2}
    
    def test_static_map_filter_reduce(self):
    ...
    def test_dynamic_map_filter_reduce(self):
        def add1(v): return v+1
        def is_gt_3(v): return v>3
        a = InputVariable('a')
        mapping = Step(map, add1, a)
    ...
    def test_iteration_over_data(self):
        a = InputVariable('a')
    ...
    def test_warn_extraneous_classes(self):
        import pytest
        class Pippo:
    ...
    def test_second_order_call(self):
        a = InputVariable('a')
        b = InputVariable('b')
        res = a.index(b)
        values = (1, 2, 3, 4, 5, 6)
    ...
    def test_give_normal_function(self):
        a = InputVariable('a')
        def add1(v): return v+1
        s = Step(add1, v=a)
        assert do_eval(s, a=3)==4
    
    def test_CAS_simple_add(self):
        a = InputVariable('a')
        s = a + 1
        assert do_eval(s, a=3, b=2) == 4
    
    def test_CAS_add_between_variables(self):
        a = InputVariable('a')
        b = InputVariable('b')
        s = a+b
    ...
    def test_CAS_sum_between_strings_asymmetric(self):
        a = InputVariable('a')
        r1 = "hello" + a
        r2 = a + "hello"
    ...
    def test_normal_partial_values_in_step(self):
        Pow = partial(Step, lambda n, p: n**p)
        a = InputVariable('a')
        p = Pow(n=a, p=2)
        assert do_eval(p, a=5) == 25
    
    def test_CAS_sums_and_prods(self):
        a = InputVariable('a')
        b = InputVariable('b')
        c = InputVariable('c')
        d = InputVariable('d')
    ...
    def test_pandas_merge_simple(self):
        import pandas as pd
    ...
    def test_pandas_merge_multiple_dynamic_keys(self):
        import pandas as pd
    ...
    def test_unorthodox_variable_names(self):
        data = {'a': 1, '--b--': 2}
        a = InputVariable('a')
        b = InputVariable('--b--')
    ...
    def test_resect_part_of_the_DAG(self):
        a = InputVariable('a')
        b = InputVariable('b')
        c = InputVariable('c')
        d = InputVariable('d')
    ...
    def test_deepcopy_cache_no_interaction(self):
        a = InputVariable('a')
        b = 1/a +1
        c = deepcopy(b)
        res = do_eval(c , a=0)
